...
of the stack. An operation that requires two values pops the topmost
item from the stack (i.e., the operation removes the most recently pushed
value from the stack), operates on the popped value and the register
...
and stores the result back in the register. For example, if we start with a
stack of [3, 6, 4] (where 4 is the topmost item in the stack) and a register
value of 7, the MULT operation transforms the stack to [3, 6] (the 4 is removed),
...
- the inital value for stack should be [], and for register 0

data structure:
- array and numbers, already defined

algorithm:
- split the string into an array based on whitespace
- create switch case statements to handle the command processing
  - push --> stack.push(register)
  - add --> register += stack.pop()
  - subtract --> register -= stack.pop()
  - multiply --> register *= stack.pop()
  - divide --> Math.floor(register /= stack.pop()
  - mod --> Math.floor(register = register % stack.pop())
  - pop --> register = stack.pop()
  - print --> console.log the register
  - default should be number --> convert to string, reassign register to this number

*/

function minilang(operationString) {
  var operations = operationString.split(' ');
  var stack = [];
  var register = 0;

  operations.forEach(function (token) {
    switch (token) {
      case 'PUSH':
        stack.push(register);
        break;
