...
# as an argument, but must not use any other methods that iterate through an
...
rules:
- return the element in the array that has the max value

algorithm:
- use each to iterate over the array
- store each return value in a variable
- compare variable with next return value
- return variable
...
# def max_by(arr)
#   temp = nil
#   arr.each { |el| temp = el if temp == nil || yield(el) > yield(temp) }
#   temp
# end
