...
--> The two disadvantages are that all properties are copied into each method, which
could be redundant and resource-intensive.  Also, there is no way to tell which
factory function created the object.
...
function createPayment(services) {
  ...
  return {
    ...
    total: function() {
      return this.amount > 0 ? this.amount : this.phone + this.internet;
    },
  };
}
...
function createInvoice(services) {
  ...
  return {
    ...
    addPayments: function() {
      payments = Array.prototype.slice.call(arguments)[0];
      var self = this;

      payments.forEach(function (payment) {
        self.totalPayment += payment.total();
      });
    },
